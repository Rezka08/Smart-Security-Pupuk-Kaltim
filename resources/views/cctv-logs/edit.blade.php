    ...
    <div class="col-md-8 mx-auto">
        <div class="card">
            <div class="card-header bg-warning text-dark">
                <h5 class="mb-0"><i class="bi bi-pencil"></i> Edit Log Sheet CCTV</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('cctv-logs.update', $cctvLog) }}" method="POST" enctype="multipart/form-data" id="cctvForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="shift" class="form-select @error('shift') is-invalid @enderror" required>
                                ...
                                <option value="A" {{ old('shift', $cctvLog->shift) == 'A' ? 'selected' : '' }}>Shift A</option>
                                <option value="B" {{ old('shift', $cctvLog->shift) == 'B' ? 'selected' : '' }}>Shift B</option>
                                <option value="C" {{ old('shift', $cctvLog->shift) == 'C' ? 'selected' : '' }}>Shift C</option>
                                <option value="D" {{ old('shift', $cctvLog->shift) == 'D' ? 'selected' : '' }}>Shift D</option>
                            </select>
                            @error('shift')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="log_time" class="form-control @error('log_time') is-invalid @enderror" 
                                   value="{{ old('log_time', $cctvLog->log_time->format('Y-m-d\TH:i')) }}" required>
                            @error('log_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="camera_location" class="form-select @error('camera_location') is-invalid @enderror" required>
                            ...
                            <option value="Zona 1" {{ old('camera_location', $cctvLog->camera_location) == 'Zona 1' ? 'selected' : '' }}>Zona 1</option>
                            <option value="Zona 2" {{ old('camera_location', $cctvLog->camera_location) == 'Zona 2' ? 'selected' : '' }}>Zona 2</option>
                            <option value="Zona 3" {{ old('camera_location', $cctvLog->camera_location) == 'Zona 3' ? 'selected' : '' }}>Zona 3</option>
                            <option value="Zona 4" {{ old('camera_location', $cctvLog->camera_location) == 'Zona 4' ? 'selected' : '' }}>Zona 4</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="incident_description" rows="3" class="form-control @error('incident_description') is-invalid @enderror" 
                                  required>{{ old('incident_description', $cctvLog->incident_description) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="action_taken" rows="3" class="form-control @error('action_taken') is-invalid @enderror" 
                                  required>{{ old('action_taken', $cctvLog->action_taken) }}</textarea>
                        @error('action_taken')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="statusAman" 
                                       value="Aman" {{ old('status', $cctvLog->status) == 'Aman' ? 'checked' : '' }} required>
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="statusTidakAman" 
                                       value="Tidak Aman" {{ old('status', $cctvLog->status) == 'Tidak Aman' ? 'checked' : '' }} required>
                    ...
                    <div class="mb-3">
                        ...
                        <div>
                            <img src="{{ Storage::url($cctvLog->evidence_photo_url) }}" alt="Evidence" class="img-thumbnail" style="max-width: 300px;">
                        </div>
                    </div>
                    @endif

                    <div class="mb-3" id="photoField" style="display: none;">
                        <label class="form-label">Upload Foto Bukti Baru <span class="text-danger" id="photoRequired">*</span></label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const hasExistingPhoto = {{ $cctvLog->evidence_photo_url ? 'true' : 'false' }};
    ...
    function togglePhotoField() {
        ...
        if (selectedStatus && selectedStatus.value === 'Tidak Aman') {
            ...
            // Jika belum ada foto, wajib upload
            if (!hasExistingPhoto) {
